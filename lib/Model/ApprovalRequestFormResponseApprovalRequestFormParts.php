...
class ApprovalRequestFormResponseApprovalRequestFormParts implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setId($id)
    {
        ...
        if (($id > 2147483647)) {
            throw new \InvalidArgumentException('invalid value for $id when calling ApprovalRequestFormResponseApprovalRequestFormParts., must be smaller than or equal to 2147483647.');
        }
        if (($id < 1)) {
    ...
    public function setOrder($order)
    {
        ...
        if (!is_null($order) && ($order < 0)) {
            throw new \InvalidArgumentException('invalid value for $order when calling ApprovalRequestFormResponseApprovalRequestFormParts., must be bigger than or equal to 0.');
        }
     ...
     * Sets type
     *
     * @param string|null $type 項目種別 (title: 申請タイトル, single_line: 自由記述形式 1行, multi_line: 自由記述形式 複数行, select: プルダウン, date: 日付, amount: 金額, receipt: 添付ファイル, section: 部門ID, partner: 取引先ID, ninja_sign_document: 契約書（NINJA SIGN連携）)
     *
     * @return self
     */
